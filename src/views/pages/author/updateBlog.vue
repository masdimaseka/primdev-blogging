...
const updateBlog = async () => {
  ...
  try {
    ...
    const response = await axios.post(baseUrl + '/blog/' + blogId.value, formData, {
      headers: {
...
<template>
  <div>
    <h1 class="text-3xl font-bold">Update Blog</h1>
    <form @submit.prevent="updateBlog" class="mt-5 grid grid-cols-1 gap-y-6">
      <baseInput
        v-model="input.title"
        inputType="text"
      ...
      <div v-if="previewImage">
        <img :src="previewImage" alt="" class="w-15 h-15 object-cover" />
      </div>
